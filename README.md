...
- A Portfolio view in the header lists your open positions with a one-click Close per position, and your resting orders with a one-click Cancel. Close only creates a gated market sell proposal for the whole position, sized from the broker's own reported quantity and simulated first; Cancel only creates a gated cancellation proposal referencing the broker's own order id. Both stop at the same confirmation card as every other order.
